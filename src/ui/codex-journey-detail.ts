...
export function mountJourneyDetail(
  ...
  const encounterWhen = step.encounterWhen ?? step.lifeStage ?? '人生的某个转折时刻';

  const root = document.createElement('div');
  root.className = 'journey-sheet-root';

  const backdrop = document.createElement('button');
  backdrop.type = 'button';
  backdrop.className = 'journey-sheet-backdrop';
  backdrop.setAttribute('aria-label', '关闭');

  const panel = document.createElement('aside');
  panel.className = 'journey-sheet-panel journey-detail';
  ...
  panel.innerHTML = `
    ...
    <button type="button" class="journey-sheet-close" aria-label="关闭">✕</button>
    <div class="journey-sheet-scroll">
      ...
      <section class="journey-detail-section journey-detail-when">
        <h3>你可能会在什么时候遇到它</h3>
        <p>你可能会在：<strong>${escapeHtml(encounterWhen)}</strong>时遇见它。</p>
      </section>
      ${
        next
  ...
  panel.querySelector('.journey-sheet-close')?.addEventListener('click', close);

  panel.querySelector('.journey-open-codex')?.addEventListener('click', () => {
    close();
    callbacks.onOpenCodex(deckId);
  });

  panel.querySelector('.journey-continue')?.addEventListener('click', () => {
    close();
