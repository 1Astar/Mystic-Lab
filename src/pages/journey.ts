...
type JourneyTab = 'all' | 'tarot' | 'xiaoliuren' | 'liuyao' | 'bazi' | 'ziwei' | 'favorites' | 'progress' | 'notes';
...
function openJourneyItem(
  ...
  openers: {
    ...
    bazi: (i: JourneyItem) => void;
    ziwei: (i: JourneyItem) => void;
  },
): void {
  if (item.system === 'tarot') openers.tarot(item);
  else if (item.system === 'xiaoliuren') openers.xiaoliuren(item);
  else if (item.system === 'liuyao') openers.liuyao(item);
  else if (item.system === 'bazi') openers.bazi(item);
  else openers.ziwei(item);
}
...
export function renderJourney(root: HTMLElement): void {
  ...
  const openers = {
    ...
    bazi: (item: JourneyItem) => openBaziDetail(item),
    ziwei: (item: JourneyItem) => openZiweiDetail(item),
  };
  ...
  function openBaziDetail(item: JourneyItem): void {
    const entry = getBaziJournalEntry(item.bazi?.id ?? '') ?? item.bazi;
    if (!entry) return;
    closeDetail();
    ...
    detail.innerHTML = `
      ...
      <header class="journal-detail-head">
        <time class="journal-detail-date">${escapeHtml(formatDate(entry.createdAt))}</time>
        <h2 class="journal-detail-question">${escapeHtml(entry.body.trim() || '（八字手札）')}</h2>
        <p class="journal-detail-meta">${escapeHtml(
          [mood ? `心情 · ${mood}` : '', entry.subjectName ? `问 · ${entry.subjectName}` : '']
            .filter(Boolean)
            .join(' · ') || '八字手札',
        )}</p>
      </header>
      ${snap ? `<p class="journal-detail-summary">${escapeHtml(snap)}</p>` : ''}
      <label class="journey-xlr-reflect-label">后来的感悟</label>
      <textarea class="question-input journey-xlr-reflect" rows="3" placeholder="写下后来对照发生了什么…">${escapeHtml(entry.reflection)}</textarea>
      <button type="button" class="btn btn-secondary" data-save-bazi style="margin-top:12px">保存感悟</button>
      <button type="button" class="btn btn-ghost" data-go-bazi-journal style="margin-top:8px">打开八字手札</button>
    `;
    detail.querySelector('.journal-detail-close')?.addEventListener('click', closeDetail);
    detail.querySelector('[data-save-bazi]')?.addEventListener('click', () => {
      const ta = detail.querySelector<HTMLTextAreaElement>('.journey-xlr-reflect');
      if (ta) {
        updateBaziJournalReflection(entry.id, ta.value);
        renderBody();
      }
    });
    detail.querySelector('[data-go-bazi-journal]')?.addEventListener('click', () => {
      navigate('/bazi/journal');
    });
    mountSiblingSummary(detail, item);
    page.appendChild(detail);
  }

  function openZiweiDetail(item: JourneyItem): void {
    const entry = getZiweiJournalEntry(item.ziwei?.id ?? '') ?? item.ziwei;
    if (!entry) return;
    closeDetail();
    ...
    detail.innerHTML = `
      ...
      <header class="journal-detail-head">
        <time class="journal-detail-date">${escapeHtml(formatDate(entry.createdAt))}</time>
        <h2 class="journal-detail-question">${escapeHtml(entry.body.trim() || '（紫微手札）')}</h2>
        <p class="journal-detail-meta">${escapeHtml(
          [mood ? `心情 · ${mood}` : '', entry.subjectName ? `问 · ${entry.subjectName}` : '']
            .filter(Boolean)
            .join(' · ') || '紫微手札',
      ...
      <label class="journey-xlr-reflect-label">后来的感悟</label>
      <textarea class="question-input journey-xlr-reflect" rows="3" placeholder="写下后来对照发生了什么…">${escapeHtml(entry.reflection)}</textarea>
      <button type="button" class="btn btn-secondary" data-save-ziwei style="margin-top:12px">保存感悟</button>
      <button type="button" class="btn btn-ghost" data-go-ziwei-journal style="margin-top:8px">打开紫微手札</button>
    `;
    detail.querySelector('.journal-detail-close')?.addEventListener('click', closeDetail);
    detail.querySelector('[data-save-ziwei]')?.addEventListener('click', () => {
      const ta = detail.querySelector<HTMLTextAreaElement>('.journey-xlr-reflect');
      if (ta) {
        updateZiweiJournalReflection(entry.id, ta.value);
        renderBody();
      }
    });
    detail.querySelector('[data-go-ziwei-journal]')?.addEventListener('click', () => {
      navigate('/ziwei/journal');
    });
    mountSiblingSummary(detail, item);
    page.appendChild(detail);
  }
  ...
  function renderTabs(): void {
    ...
    const defs: { id: JourneyTab; label: string; count?: number }[] = [
      ...
      },
      {
        id: 'bazi',
        label: '八字手札',
        count: items.filter((i) => i.system === 'bazi').length,
      },
      {
        id: 'ziwei',
        label: '紫微手札',
        count: items.filter((i) => i.system === 'ziwei').length,
      },
  ...
  function renderBody(): void {
    ...
    if (items.length === 0) {
      ...
      const hints = {
        all: '还没有旅程记录。完成一次塔罗 / 小六壬 / 六爻占问，或写一条八字 / 紫微手札后，会出现在这里。',
        tarot: '还没有塔罗记录。',
        xiaoliuren: '还没有小六壬记录。',
        liuyao: '还没有六爻记录。去起卦并保存到「我的卦象」后会出现在这里。',
        bazi: '还没有八字手札。去八字手札写一条体感后会出现在这里。',
        ziwei: '还没有紫微手札。去紫微手札写一条体感后会出现在这里。',
      ...
      if (active === 'all' || active === 'bazi') {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-ghost';
        btn.textContent = '去八字手札';
        btn.addEventListener('click', () => navigate('/bazi/journal'));
        actions.appendChild(btn);
      }
      if (active === 'all' || active === 'ziwei') {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-ghost';
        btn.textContent = '去紫微手札';
        btn.addEventListener('click', () => navigate('/ziwei/journal'));
        actions.appendChild(btn);
      }
          ...
          item.system === 'tarot' && item.tarot?.subjectName
            ...
            : item.system === 'liuyao' && item.liuyao?.subjectName
              ...
              : item.system === 'xiaoliuren' && item.xiaoliuren?.subjectName
                ...
                : item.system === 'bazi' && item.bazi?.subjectName
                  ? `<p class="journey-badge">问 · ${escapeHtml(item.bazi.subjectName)}${
                      item.bazi.sceneTags?.length
                        ? ` · ${item.bazi.sceneTags.map((t) => `#${t}`).join(' ')}`
                        : ''
                    }</p>`
                  : item.system === 'ziwei' && item.ziwei?.subjectName
                    ? `<p class="journey-badge">问 · ${escapeHtml(item.ziwei.subjectName)}${
                        item.ziwei.sceneTags?.length
                          ? ` · ${item.ziwei.sceneTags.map((t) => `#${t}`).join(' ')}`
                          : ''
                      }</p>`
                    : ''
        }
